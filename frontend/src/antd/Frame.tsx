...
import { useWhoamiQuery, useLoginUrlQuery, useLogoutMutation } from '../api'
...
export const Frame: FC<FrameProps> = (props: FrameProps) => {
  ...
  const officer = () => {
    navigate(pathGen.get(routes.officer))
  }
  ...
  let menuItems: MenuProps['items'] = [{ key: routes.antbase, label: 'Home' }]
  ...
  const onSelect: SelectEventHandler = ({ item, key, keyPath, domEvent }) => {
    switch (key) {
      case routes.answer:
        navigate(pathGen.get(routes.answer))
        break
      case routes.screen:
        navigate(pathGen.get(routes.screen))
        break
      default:
        navigate(pathGen.get())
        break
    }
  }
  ...
  return (
    <Layout className="layout">
      ...
      <Footer style={{ textAlign: 'center' }}>
        ...
        <br />
        <Link to={pathGen.get(routes.privacyStatement)}>Privacy Statement</Link>
        <Divider type="vertical" />
        <Link to={pathGen.get(routes.termsOfUse)}>Terms of Use</Link>
        <Divider type="vertical" />
